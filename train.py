...
dep_vocab = Vocab.load_vocab(args.vocab_dir + '/vocab_dep.vocab')      # dep-rel 依存关系
...
# word_emb, list[token_vocab]
word_emb = load_pretrained_embedding(glove_dir=args.glove_dir, word_list=token_vocab.itos)
assert len(word_emb) == len(token_vocab)    # 判断词数相同
assert len(word_emb[0]) == args.emb_dim     # 判断嵌入维度相同
word_emb = torch.FloatTensor(word_emb)      # convert to tensor 此时word_emb,shape[vocab_emb, emb_dim=300]
...
# build model
# emb_matrix, shape[vocab_emb, emb_dim=300]
#
...
for epoch in range(1, args.num_epoch+1):
    ...
    # i是批次的索引，batch是当前批次的数据，每一个batch中含有9个tuple，每个tuple中是分别代表tok, asp, pos, head, deprel, post, mask, length, label的张量
    # 其中，tok到mask均是二维张量:shape[batch_size, 各自的max_len]，length&label是一维张量:shape[batch_size,]
    ...
    print("train_loss: {}, test_loss: {}, train_acc: {}, test_acc: {}, f1_score: {}".format( \
