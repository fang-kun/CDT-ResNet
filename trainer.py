...
class GCNTrainer(object):
    ...
    def update(self, batch):
        ...
        inputs = batch[0:8]     # 取tok, asp, pos, head, deprel, post, mask, l
        label = batch[-1]   # 获取label

        # step forward
        self.model.train()
        self.optimizer.zero_grad()
        # 输入 inputs,list[8]:from tok to l
        # 输出 gcn_outputs,shape[batch_size=32,hidden_size=50]  logits,shape[batch_size=32, num_class=3]
